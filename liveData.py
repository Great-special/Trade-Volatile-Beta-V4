...
def getTradeAble_Data():
    ...
    for symbol in list_symbols:
        # H1_data = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H1, 0, 720)
        M5_data = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_M5, 0, 8640)
        
        # create DataFrame out of the obtained data
       
        df_M5 = pd.DataFrame(M5_data, columns=['time', 'open', 'high', 'low', 'close'])
        # df_H1 = pd.DataFrame(H1_data, columns=['time', 'open', 'high', 'low', 'close'])
        symbols_Data.append({
                'symbolName': symbol,
                'df_M5' : df_M5,
            })
        ...
        # df_H1.to_excel(f'{symbol} h1.xlsx')
