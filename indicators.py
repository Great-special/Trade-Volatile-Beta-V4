...
def atr(df):
    ...
    '''
    Df = df
    Df['ATR'] = ta.atr(high=Df.high, low=Df.low, close=Df.close, length=2) # 10 
    ATR_val = list(Df['ATR'])
...
def MA(df):
    df['MA'] = ta.sma(df.close, length=200, method='Simple') # 10
    return df['MA']

def LowsMins(df):
    df['Mins'] = df['low'].rolling(window=7).min()
    return df['Mins']

def HighsMaxs(df):
    df['Maxs'] = df['high'].rolling(window=7).max()
    return df['Maxs']
...
def market_direction(df, track, back_candles):
    ...
    for index in range(track - back_candles, 0, track + 1):
        if df.low[index] <= df.MA[index]:
            upTrend = 0
        if df.high[index] >= df.MA[index]:
            downTrend = 0
...
def priceRejection(l, open_, high, close, low, high_diff, body_diff, low_diff, h_ratio, lo_ratio):
    ...
    mid_diff = abs(high[row] - low[row])

    body_diff[row] = abs(open_[row] - close[row])
    low_diff[row] = min(open_[row], close[row]) - low[row]

    body_type1 = open_[row-2] > close[row-2]
    body_type2 = open_[row-1] > close[row-1]
    body_type3 = open_[row-2] < close[row-2]
    body_type4 = open_[row-1] < close[row-1]
    
    current1 = close[row] < open_[row]
    current2 =close[row] > open_[row]
    
    
    c2 = close[row-1] > open_[row]
    
    mybodyDiff = mid_diff / 7
    ...
    #sell signal  shooting star
    if h_ratio[row] >= 2.5 and low_diff[row] <= body_diff[row] and body_diff[row]>= mybodyDiff and body_type3 and body_type4:
        return 1
        
    #buy signal  Hammer
    elif lo_ratio[row] >= 2.5 and high_diff[row] <= body_diff[row] and body_type1 and body_type2 and c2 and body_diff[row]>= mybodyDiff:
        return 2       
    else:
        return 0


def InverseRejection(l, open_, high, close, low, high_diff, body_diff, low_diff, h_ratio, lo_ratio):
    row = l
    ...
    mid = body_diff[row-1] / 2
    mid_lvl = max(open_[row-1] , close[row-1]) - mid
    
    c1 = open_[row-1] < close[row-1]
    c2 = close[row] >= mid_lvl
    c3 = open_[row-1] > close[row-1]
    ...
    #sell signal  hanging man
    if lo_ratio[row] >= 2.5 and high_diff[row] <= body_diff[row] and close[row] < open_[row] and body_diff[row]>= mybodyDiff and c1 and c2:
        return 1
        
    #buy signal  inverse Hammer
    elif h_ratio[row] >= 2.5 and low_diff[row] <= body_diff[row] and close[row] > open_[row] and body_diff[row]>= mybodyDiff and c3:
        return 2       
    else:
        return 0
...
def DarkLine(l, open_, close, low,  high, body_diff): # works more on buys 
    ...
    higher_close = close[row-1] > close[row-2]
    higher_high = high[row-1] > high[row-3] 
    lower_close = close[row-1] < close[row-2]
    lower_low = low[row-1] < low[row-3]
   
    
    #selling signal Dark Cloud
    if (body_diff[row] > body_diffmin and body_diff[row-1] > body_diffmin and higher_close and higher_high and
    ...
    if (body_diff[row] > body_diffmin and body_diff[row-1] > body_diffmin and lower_low and lower_close and
        open_[row] < close[row] and open_[row-1] > close[row] and open_[row] < close[row-1] and close[row] >= mid_lvl
        ):
...
def Engulfing(l, close, open_, body_diff):
    ...
    #selling signal
    if (body_diff[row] > body_diffmin and body_diff[row-1] > body_diffmin and higher_close and higher_high and
        open_[row - 1] < close[row - 1] and open_[row] > close[row] and close[row-1] <= open_[row]):
        return 1
    
    #buying signal
    elif(body_diff[row] > body_diffmin and body_diff[row-1] > body_diffmin and lower_low and lower_close and 
        open_[row-1] > close[row-1] and open_[row] < close[row] and close[row-1] <= open_[row]): 
        return 2
...
def Harami(l, close, open_, low,  high, body_diff): # works more on sells but better with a buy
    ...
    # selling signal
    if (open_[row-1] < close[row-1] and higher_close and higher_high  and open_[row] > close[row] and close[row-1] > open_[row] and open_[row-1] < close[row]):
        return 1
    

    # buying signal
    if (open_[row-1] > close[row-1] and lower_close and lower_low and open_[row] < close[row] and close[row-1] < open_[row] and open_[row-1] > close[row]):
        return 2
